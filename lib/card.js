...
const EMBEDDED_FONT_FAMILY = "QuoteEmbeddedSans";
const EMBEDDED_FONT_DATA = fs
  .readFileSync(path.join(__dirname, "../fonts/DejaVuSans.ttf"))
  .toString("base64");

function embeddedFontStyle() {
  return `<style>@font-face{font-family:"${EMBEDDED_FONT_FAMILY}";src:url(data:font/ttf;base64,${EMBEDDED_FONT_DATA}) format("truetype");font-weight:100 900;font-style:normal;}text{font-family:"${EMBEDDED_FONT_FAMILY}" !important;}</style>`;
}
...
function footer(quote, layout, style, options = {}) {
  ...
  return `
    ...
    <text x="${contentX}" y="${layout.footerY + 15}" fill="${style.ink}" font-family="DejaVu Sans" font-size="25" font-weight="700">${character}</text>
    <text x="${contentX}" y="${layout.footerY + 50}" fill="${style.ink}" opacity=".72" font-family="DejaVu Sans" font-size="19">${anime}</text>
  `;
}
...
function editorialCard(quote, style, layout) {
  return `
    ...
    <text x="103" y="${layout.quoteTop - 20}" fill="${style.accent}" font-family="DejaVu Serif" font-size="74">“</text>
...
function cinemaCard(quote, style, layout) {
  return `
    ...
    <text x="91" y="${layout.quoteTop - 20}" fill="${style.accent}" font-family="DejaVu Serif" font-size="74">“</text>
...
function polaroidCard(quote, style, layout) {
  return `
    ...
    <text x="95" y="${layout.quoteTop - 20}" fill="${style.accent}" font-family="DejaVu Serif" font-size="74">“</text>
...
function galleryCard(quote, style, layout) {
  ...
  return `
    ...
    <text x="${centerX}" y="${layout.quoteTop - 20}" fill="${style.accent}" font-family="DejaVu Serif" font-size="74" text-anchor="middle">“</text>
    ...
    <text x="${centerX}" y="${layout.footerY + 15}" fill="${style.ink}" font-family="DejaVu Sans" font-size="25" font-weight="700" text-anchor="middle">${escapeXml(truncateLabel(quote.character, 58))}</text>
    <text x="${centerX}" y="${layout.footerY + 50}" fill="${style.ink}" opacity=".72" font-family="DejaVu Sans" font-size="19" text-anchor="middle">${escapeXml(truncateLabel(quote.anime))}</text>
  `;
}

function quoteCardSvg(quote, styleName = "editorial", options = {}) {
  ...
  const fontStyle = options.embedFont ? embeddedFontStyle() : "";
